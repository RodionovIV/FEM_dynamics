...
#Основная вычислительная функция
    def process_dinamic(self, data):
        ...
        pred_norm = 10000
        while True:
        #while np.linalg.norm(disp-U_final) >= 1e-3:
            U_final = 0
            t = nt*dt
            for evector, eval in zip(evectors_T, evals):
                ...
                y_i = y_i0*np.cos(eval**(0.5) * t) + f_i / (m_i * eval)
                U_final = U_final + y_i * evector.T
                #print(evector.shape)
                #print(U_final[:2])
            #print('Tut')
            #U_final = U_final.reshape((-1,dim))
            #sigma_calculate(U_final,data,nt)
            nt+=1
        ...
        U_final = U_final.reshape((-1,dim))
        sigma_xx,sigma_yy,sigma_xy = sigma_calculate(U_final,data)
